...
import { Fish, Book, MessageCircle, ShoppingBag, Stethoscope, Home, Menu, Search, LogIn, UserCircle, Settings, Activity } from 'lucide-react';
...
export function WebsiteHeader({ activeSection, onSectionChange, user, onLoginClick, onLogout, onSwitchToGuest }: WebsiteHeaderProps) {
  const baseNavigationItems = [
    ...
    { id: 'monitoring', label: 'Monitoring', icon: Activity, badge: 'IoT' },
  ...
  const navigationItems = user?.role === 'admin' 
    ? [...baseNavigationItems, { id: 'admin', label: 'Admin', icon: Settings, badge: 'Admin' }]
    : baseNavigationItems;
            ...
            {user ? (
              <div className="flex items-center gap-2">
                ...
                <UserProfileDropdown 
                  ...
                  onNavigateToAdmin={() => onSectionChange('admin')}
                />
              </div>
            ) : (
              <Button
